#include "Sphere.h"
bool Sphere::Hit(const ray_t & ray, float minDistance, float maxDistance,
	raycastHit_t & raycastHit) {
	// compute direction vector (ray origin - sphere center)
	glm::vec3 oc = ray.origin - position;
	// quadratic coefficients for raysphere intersection
	// a coefficient is a number that multiplies a variable in a mathematical
	//expression
		...
		if (discriminant >= 0) {
			// quadratic formula gives possible raysphere intersection distances.
			// t = (-b sqrt(discriminant)) / (2a)
			// solve quadratic for the nearest intersection: t = (-b -
